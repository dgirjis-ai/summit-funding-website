    ...
    <section id="about" className="py-24 md:py-32 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="max-w-3xl mx-auto text-center mb-16">
          ...
          >
            Your Bridge to Business Funding
          </motion.h2>
          <motion.p
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
        ...
        >
          {features.map((f) => (
            <motion.div
              key={f.title}
              variants={item}
              className="group relative bg-ice rounded-2xl p-8 hover:bg-navy transition-all duration-500 hover:shadow-2xl"
            >
              <div className="text-navy group-hover:text-white transition-colors mb-5">
                {f.icon}
              </div>
              <h3 className="text-xl font-bold text-navy group-hover:text-white transition-colors mb-3">
                {f.title}
              </h3>
              <p className="text-steel group-hover:text-white/70 transition-colors leading-relaxed">
                {f.desc}
              </p>
            </motion.div>
          ))}
